...
def _mk_app(token: Union[str, None] = None) -> tuple[Flask, StatsStore]:
    app = Flask(__name__)
    stats = StatsStore()
    bp = make_ui_blueprint(stats, bearer_token=token, ui_prefix="/spy")
...
def test_auth_check_forbidden() -> None:
    ...
    r2 = client.get("/spy/metrics.json", headers={"Authorization": "Bearer secret"})
